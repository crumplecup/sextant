...
use geo_types::{LineString, Point, Coord, Polygon, MultiPolygon};
...
impl Convert<MultiPolygon> {
    pub fn multipolygon(self) -> galileo_types::impls::MultiPolygon<Point2d> {
        let conv = self.0.iter().map(|v| Convert::new(v.clone())).collect::<Vec<Convert<Polygon>>>();
        let parts = conv.par_iter().cloned().map(|v| v.polygon()).collect::<Vec<galileo_types::impls::Polygon<Point2d>>>();
        galileo_types::impls::MultiPolygon { parts }

    }

    pub fn bounded_multipolygon(self) -> (galileo_types::impls::MultiPolygon<Point2d>, galileo_types::cartesian::Rect<f64>) {
        let mut boundaries = Vec::new();
        let conv = self.0.iter().map(|v| Convert::new(v.clone())).collect::<Vec<Convert<Polygon>>>();
        let parts = conv.iter().cloned().map(|v| {
...
impl Convert<Polygon> {
    ...
    pub fn bounded_polygon(self) -> (galileo_types::impls::Polygon<Point2d>, Option<galileo_types::cartesian::Rect<f64>>) {
